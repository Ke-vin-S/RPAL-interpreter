...
public class IntGreaterThanOrEqual implements BinaryOperation {
    @Override
    public StackElement apply(StackElement left, StackElement right) {
        if (!(left instanceof DataStackElement l) || !(right instanceof DataStackElement r)) {
            throw new IllegalArgumentException("Greater-than-or-equal comparison requires DataStackElements.");
        }

        if (l.getDataType() != DataStackElement.Type.INT || r.getDataType() != DataStackElement.Type.INT) {
            throw new IllegalArgumentException("Greater-than-or-equal comparison requires integer operands.");
        }
